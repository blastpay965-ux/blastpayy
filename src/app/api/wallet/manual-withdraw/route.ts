...
export async function POST(request: Request) {
  ...
  if (isRateLimited(`manual-withdraw:${ip}`, 5, 60_000)) return rateLimitedResponse();
  ...
  try {
    ...
    const { amount, bankName, accountNumber } = sanitizeInput(raw);
    const val = parseFloat(amount);
    const bankStr = (bankName || '').trim();
    ...
    // Step 1: Deduct balance atomically (Do NOT log transaction yet)
    ...
    const uniqueRef = `BANK: ${bankStr} | ACCT: ${acctStr}`;

    // Step 3: Create the Pending Withdrawal Transaction
    await createPendingTransaction(user.id, val, uniqueRef, 'withdrawal');

    pushTelemetryLog('WALLET', `💸 WITHDRAWAL REQUEST: Player [${profile.username}] requested ₦${val.toLocaleString()} to ${bankStr} (${acctStr}). Status: PENDING.`);
